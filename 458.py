...
# ============================================================================
# PART 1: 2D PHYSICS ENGINE (World Model)
# ============================================================================

@dataclass
class Particle:
    """A single physical particle in the 2D world."""
    pos: np.ndarray
    vel: np.ndarray
    radius: float = 0.08
    mass: float = 1.0
...
class PhysicsEngine:
    ...
    Features: gravity, damping, boundary collision, particle-particle collision.
    """
    def __init__(self, gravity: Tuple[float, float] = (0.0, 0.0), 
        ...
        self.bounds = [0.0, 1.0, 0.0, 1.0]  # [x_min, x_max, y_min, y_max]

    def set_particles(self, positions: List[List[float]], radii: Optional[List[float]] = None):
        """Initialize particles at given positions."""
        self.particles = []
    ...
    def step(self, actions: Optional[List[List[float]]] = None):
        """Advance physics by one timestep with external forces."""
        if actions is None:
        ...
        for i, p in enumerate(self.particles):
            ...
            # Boundary collision (coefficient of restitution = 0.7)
    ...
    def get_state_vector(self) -> np.ndarray:
        """Flatten state: [x1, y1, vx1, vy1, x2, y2, vx2, vy2, ...]."""
        state = []
    ...
    def set_state_vector(self, state: np.ndarray):
        """Restore state from flattened vector."""
        idx = 0
        for p in self.particles:
            p.pos[0] = state[idx]
            p.pos[1] = state[idx + 1]
    ...
    def clone_and_simulate(self, state: np.ndarray, actions: List[List[float]], 
        ...
        """Clone state, simulate forward, return final state (non-destructive)."""
...
# ============================================================================
# PART 2: DEEPSEEK-VL PERCEPTION (with Mock Fallback)
# ============================================================================

class DeepSeekVLPerceptor:
    """Perception module using DeepSeek-VL to parse scene into JSON."""
    ...
    def __init__(self, model_name: str = "deepseek-ai/deepseek-vl-7b-base", 
        ...
        if not use_mock:
            try:
                ...
                print(f"Loading DeepSeek-VL from {model_name}...")
    ...
    def perceive(self, image_path: str, instruction: str = "") -> Dict[str, Any]:
        """Perceive a scene from an image and return a structured scene graph."""
        if self.use_mock:
    ...
    def _mock_perceive(self) -> Dict[str, Any]:
        """Generate random scene for testing without GPU."""
...
# ============================================================================
# PART 3: BRIDGE (Scene Graph → Physics State)
# ============================================================================

class SceneBridge:
    """
    Bridges semantic scene understanding to physical state vectors.
    Extracts positions from JSON and maps them to physics engine states.
    """
    
    @staticmethod
    def scene_to_state(scene: Dict[str, Any], num_particles: int = 1) -> np.ndarray:
        """
        Convert scene graph to physics state vector.
        Returns: [x1, y1, 0, 0, x2, y2, 0, 0, ...] (velocities initialized to 0)
        """
        state = []
    ...
    def get_target(scene: Dict[str, Any]) -> np.ndarray:
        """Extract target position from scene graph."""
...
class MPCPlanner:
    ...
    Samples random action sequences, simulates them in the physics engine,
    and selects the best first action.
    """
    ...
    def plan(self, current_state: np.ndarray, target_position: np.ndarray) -> np.ndarray:
        """
        Plan the next action (force [fx, fy]) to reach target.
        current_state: full state vector [x1,y1,vx1,vy1, x2,y2,vx2,vy2, ...]
        ...
        for _ in range(self.num_samples):
            ...
            # Cost: distance to target + action effort penalty
...
# ============================================================================
# PART 5: MAIN AGENT LOOP
# ============================================================================

class LLMPhysicalAgent:
    """
    Complete agent that combines:
    - DeepSeek-VL perception
    - Physics engine (world model)
    - Bridge (semantic → physical)
    - MPC planner
    """
    ...
    def step(self, image_path: str) -> Dict[str, Any]:
        """
        Perform one closed-loop step:
        1. Perceive scene from image
        2. Convert to physics state
        3. Plan action using MPC
        4. Execute action in physics engine
        5. Return status
        """
        self.step_count += 1
    ...
    def run(self, image_path: str, max_steps: int = 30) -> bool:
        """
        Run the agent loop until target is reached or max_steps exceeded.
        Returns True if target reached.
        """
        self.reset()
        print("=" * 60)
        print("LLM Physical Agent - Starting Control Loop")
        print("=" * 60)
        ...
        for step in range(max_steps):
            ...
            print(f"\n[Step {status['step']:2d}] "
                  f"Pos: ({status['position'][0]:.3f}, {status['position'][1]:.3f}) "
                  f"→ Target: ({status['target'][0]:.3f}, {status['target'][1]:.3f}) "
                  f"| Dist: {status['distance']:.4f} "
                  f"| Action: ({status['action'][0]:+.3f}, {status['action'][1]:+.3f})")
            
            if status["reached"]:
                print(f"\n🎯 Target reached at step {status['step']}!")
                return True
        
        print(f"\n❌ Failed to reach target within {max_steps} steps.")
        return False
